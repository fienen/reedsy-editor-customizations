...
export const removeUpsellPanels = () => {
    ...
    if (premiumAdBanner && statsContainer && statsContainer.children.length > 1 && actionButtons.length > 0 && premiumDesc && premiumDesc.length > 0) {
        ...
        if (statPanelsToRemove.length > 1) {           
            ...
            console.log('[Reedsy Editor Customizations] Removed "Most Used" stat panels.');
        }
        ...
        if(removeUpsellReloads > 10) {
            ...
            console.log('[Reedsy Editor Customizations] Upsell ads not detected, retrying in 2 seconds. (Attempt ' + removeUpsellReloads + ' of 10)');
            
            setTimeout(removeUpsellPanels, 2000);
        }
    }
};